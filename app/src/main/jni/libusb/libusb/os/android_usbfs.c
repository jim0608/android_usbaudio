...
//		usbfs_path = "/dev/bus/usb";
	...
	if (UNLIKELY(!usbfs_path)) {
		LOGE("could not find usbfs:%p ",usbfs_path);
...
//	LOGE("handling completion status %d of iso urb %d/%d",
//		urb->status, urb_idx, num_urbs);

	if((urb->status) == -ESHUTDOWN){
		LOGE("device removed with status -108");
		goto out;
	}
	...
	for (i = 0; i < urb->number_of_packets; i++) {
		...
		case -ENOSR:
		case -EXDEV:
			LOGE("low-level USB error %d", urb_desc->status);
	...
	LOGE("CANCEL: urb num_retired %d num_urbs %d", tpriv->num_retired,num_urbs);
	tpriv->num_retired++;
    LOGE("usbfs: urb status %d", __LINE__);
//	LOGE("CANCEL: urb status %d", tpriv->reap_action);
 	if (UNLIKELY(tpriv->reap_action != NORMAL)) { /* cancelled or submit_fail */
		LOGE("CANCEL: urb status %d", urb->status);
		LOGE("CANCEL: urb num_retired %d num_urbs %d", tpriv->num_retired,num_urbs);

		if (tpriv->num_retired == num_urbs) {
			LOGE("CANCEL: last URB handled, reporting");
			free_iso_urbs(tpriv);
			if (tpriv->reap_action == CANCELLED) {
				...
				LOGE("usbfs: urb status %d", __LINE__);
				usbi_mutex_unlock(&itransfer->lock);
				LOGE("call usbi_handle_transfer_completion because of cancel");
    ...
    if (urb_idx == num_urbs && (urb->status) != -ESHUTDOWN) {
//        LOGE("last URB in transfer when device removed --> complete!");
        free_iso_urbs(tpriv);
        usbi_mutex_unlock(&itransfer->lock);
		LOGE("usbfs: urb status %d", __LINE__);
