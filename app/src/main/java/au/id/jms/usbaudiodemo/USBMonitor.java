...
import au.id.jms.usbaudiodemo.DeviceFilter;
import au.id.jms.usbaudiodemo.USBVendorId;
    ...
    public synchronized void register() throws IllegalStateException {
        ...
        if (mPermissionIntent == null) {
            ...
            if (context != null) {
                ...
                Log.d(TAG, "register: registerReceiver");
...
//			if (DEBUG) Log.i(TAG, "unregister:");
    ...
    public List<UsbDevice> getDeviceList() throws IllegalStateException {
        if (destroyed) throw new IllegalStateException("already destroyed");
        List<UsbDevice> deviceLists = new ArrayList<>();
        final HashMap<String, UsbDevice> deviceList = mUsbManager.getDeviceList();
        for (UsbDevice device : deviceList.values()) {
            boolean match = false;
            if (mDeviceFilters != null) {
                for (DeviceFilter deviceFilter : mDeviceFilters) {
                    if (deviceFilter.matches(device) && !deviceFilter.isExclude) {
                        match = true;
                        break;
                    }
                }
...
//        return getDeviceList(mDeviceFilters);
    ...
    public List<UsbDevice> getDeviceList(final List<DeviceFilter> filters) throws IllegalStateException {
        ...
        String fileName = mWeakContext.get().getFilesDir().getAbsolutePath() + "/USBCamera/failed_devices.txt";
//        Log.d(TAG, fileName);
...
//		if (DEBUG) Log.v(TAG, "requestPermission:device=" + device);
    ...
    public UsbControlBlock getDevice(final UsbDevice device) throws SecurityException {
        if (hasPermission(device)) {
            return mCtrlBlocks.get(device);
        } else {
            throw new SecurityException("has no permission");
        }
    }
    ...
    private final void processCancel(final UsbDevice device) {
        if (destroyed) return;
        if (DEBUG)
            Log.v(TAG, "processCancel:" + android.util.Log.getStackTraceString(new Throwable()));
...
//			}
        ...
        public synchronized void close() {
            if (DEBUG) Log.i(TAG, "UsbControlBlock#close:", new Throwable());
